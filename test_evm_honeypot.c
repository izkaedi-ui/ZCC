...
static EvmNode nodes[NUM_NODES] = {
    {{0.0f, 0.0f, 0.0f}, 6.0f, "0x00_HONEY_BAIT", "BALANCE: 2,500 WETH (OPEN)", "#00ff66"}, // Honey bait (Green)
    {{-16.0f, 8.0f, -5.0f}, 3.8f, "0x15_VICTIM_DEPOSIT", "PAYABLE / USER_INFLOW", "#00ffff"}, // Deposit (Cyan)
    {{15.0f, 10.0f, 5.0f}, 4.2f, "0x2A_LOCKED_WITHDRAW", "TRANSFER / REVERT_TRAP", "#ff0033"}, // Withdraw lock (Red)
    {{12.0f, -12.0f, -6.0f}, 3.5f, "0x3C_HIDDEN_BACKDOOR", "ONLY_OWNER / RECOVERY", "#ffaa00"}, // Backdoor (Orange)
    {{-14.0f, -10.0f, 8.0f}, 4.0f, "0x4E_OWNER_SIPHON", "TRANSFER / OWNER_OUTFLOW", "#ff00ff"} // Siphon (Magenta)
};

#define NUM_EDGES 5
static EvmEdge edges[NUM_EDGES] = {
    {1, 0, 0.8f}, // Deposit -> Honey (funds go in!)
    {0, 2, 0.7f}, // Honey -> Withdraw (attempted exit reverts!)
    {3, 0, 0.7f}, // Backdoor -> Honey (owner keys)
    {0, 4, 0.8f}, // Honey -> Siphon (owner sucks funds out!)
    {4, 1, 0.6f}  // Siphon -> Deposit
};
...
static void get_node_pos(int index, float phase, Vec3* pos, float* radius) {
    ...
    *pos = base.pos;
    ...
    if (index == 0) {
        // Honey bait orbits slightly in a small circle, mimicking temptation
        pos->x = 2.0f * cosf(phase * 2.0f * M_PI);
        pos->y = 2.0f * sinf(phase * 2.0f * M_PI);
    } else {
        // Dynamic outer nodes spiral and rotate
...
static float honey_scene_sdf(Vec3 p, ChaosPhasor* ph) {
    Vec3 pr = p;
    ...
    // 3. Hidden negative Menger Sponge subtracting and carving spiky gaps inside the central Honey Bait
...
static Vec3 honey_get_normal(Vec3 p, ChaosPhasor* ph) {
    Vec3 e = {EPSILON, 0.0f, 0.0f};
    Vec3 n = {
        honey_scene_sdf((Vec3){p.x + e.x, p.y, p.z}, ph) - honey_scene_sdf((Vec3){p.x - e.x, p.y, p.z}, ph),
        honey_scene_sdf((Vec3){p.x, p.y + e.x, p.z}, ph) - honey_scene_sdf((Vec3){p.x, p.y - e.x, p.z}, ph),
        honey_scene_sdf((Vec3){p.x, p.y, p.z + e.x}, ph) - honey_scene_sdf((Vec3){p.x, p.y, p.z - e.x}, ph)
    };
    normalize(&n);
    return n;
}

static float honey_ray_march(Ray r, ChaosPhasor* ph, Vec3* hit, Vec3* normal) {
    float depth = 0.0f;
    for (int i = 0; i < MAX_STEPS; ++i) {
        Vec3 p = {r.ro.x + r.rd.x * depth, r.ro.y + r.rd.y * depth, r.ro.z + r.rd.z * depth};
        float dist = honey_scene_sdf(p, ph);
        depth += dist;
        if (dist < SURF_DIST) {
            *hit = p;
            *normal = honey_get_normal(p, ph);
            return depth;
        }
...
int main() {
    ...
    ZccSvgNode* stop1 = svg_stop(); svg_set_offset(stop1, "0%"); svg_set_stop_color(stop1, "#00ff66"); svg_add_child(glowGrad, stop1);
    ...
    for (int f = 0; f < num_frames; f++) {
        ...
        for (int y = 0; y < height; y += grid_step) {
            for (int x = 0; x < width; x += grid_step) {
                ...
                Vec3 hit, normal;
                float d = honey_ray_march((Ray){ro, rd}, &ph, &hit, &normal);
    ...
    svg_set_attr(corner_lbl, "fill", "#00ff66");
    svg_set_content(corner_lbl, "🔱 ZKAEDI HONEYPOT: TEMPTATION TRAP EXPLOIT CORE ACTIVE");
    svg_add_child(svg, corner_lbl);

    char* xml = svg_to_string(svg);
    FILE* fp = fopen("test_evm_honeypot.svg", "w");
    if (fp) {
        fputs(xml, fp);
        fclose(fp);
        printf("SUCCESS: Generated test_evm_honeypot.svg!\n");
    }
