...
static EvmNode nodes[NUM_NODES] = {
    {{0.0f, 0.0f, 0.0f}, 5.5f, "0x00_DAO_TREASURY", "BALANCE: 15,000,000 WETH", "#00ff66"}, // Treasury (Green)
    {{-15.0f, 12.0f, -5.0f}, 3.5f, "0x08_VOTE_POWER", "DELEGATE / FLASH_VOTE", "#00ffff"}, // Flash vote (Cyan)
    {{16.0f, 10.0f, 5.0f}, 4.0f, "0x1A_MALICIOUS_PROP", "PROPOSE / DRAIN_TREASURY", "#ff007f"}, // Proposal (Hot Pink)
    {{10.0f, -12.0f, -6.0f}, 3.8f, "0x2F_TIMELOCK_BYPASS", "QUEUE / EXPLOIT_DELAY", "#ffaa00"}, // Timelock (Orange)
    {{-14.0f, -10.0f, 8.0f}, 4.2f, "0x3D_EXECUTE", "EXECUTE / WITHDRAW_TREASURY", "#ff0033"} // Execute (Red)
};

#define NUM_EDGES 5
static EvmEdge edges[NUM_EDGES] = {
    {1, 2, 0.7f}, // Votes -> Proposal
    {2, 3, 0.8f}, // Proposal -> Timelock
    {3, 4, 0.7f}, // Timelock -> Execute
    {4, 0, 0.9f}, // Execute -> Treasury (Hostile infiltration!)
    {0, 1, 0.6f}  // Treasury -> Votes (circular lock)
};
...
static void get_node_pos(int index, float phase, Vec3* pos, float* radius) {
    ...
    *pos = base.pos;
    ...
    if (index == 0) {
        // Treasury pulses under pressure from the governance attack
        *radius = base.radius * (1.0f + 0.20f * sinf(phase * 4.0f * M_PI));
    } else {
        // Governance path orbits Treasury vertically like a double-ring
...
static float gov_scene_sdf(Vec3 p, ChaosPhasor* ph) {
    Vec3 pr = p;
    ...
    // 3. Spiky Menger Sponge shielding the DAO treasury
...
static Vec3 gov_get_normal(Vec3 p, ChaosPhasor* ph) {
    Vec3 e = {EPSILON, 0.0f, 0.0f};
    Vec3 n = {
        gov_scene_sdf((Vec3){p.x + e.x, p.y, p.z}, ph) - gov_scene_sdf((Vec3){p.x - e.x, p.y, p.z}, ph),
        gov_scene_sdf((Vec3){p.x, p.y + e.x, p.z}, ph) - gov_scene_sdf((Vec3){p.x, p.y - e.x, p.z}, ph),
        gov_scene_sdf((Vec3){p.x, p.y, p.z + e.x}, ph) - gov_scene_sdf((Vec3){p.x, p.y, p.z - e.x}, ph)
    };
    normalize(&n);
    return n;
}

static float gov_ray_march(Ray r, ChaosPhasor* ph, Vec3* hit, Vec3* normal) {
    float depth = 0.0f;
    for (int i = 0; i < MAX_STEPS; ++i) {
        Vec3 p = {r.ro.x + r.rd.x * depth, r.ro.y + r.rd.y * depth, r.ro.z + r.rd.z * depth};
        float dist = gov_scene_sdf(p, ph);
        depth += dist;
        if (dist < SURF_DIST) {
            *hit = p;
            *normal = gov_get_normal(p, ph);
            return depth;
        }
...
int main() {
    ...
    ZccSvgNode* stop1 = svg_stop(); svg_set_offset(stop1, "0%"); svg_set_stop_color(stop1, "#ff0033"); svg_add_child(glowGrad, stop1);
    ZccSvgNode* stop2 = svg_stop(); svg_set_offset(stop2, "100%"); svg_set_stop_color(stop2, "#00ff66"); svg_add_child(glowGrad, stop2);
    ...
    for (int f = 0; f < num_frames; f++) {
        ...
        for (int y = 0; y < height; y += grid_step) {
            for (int x = 0; x < width; x += grid_step) {
                ...
                Vec3 hit, normal;
                float d = gov_ray_march((Ray){ro, rd}, &ph, &hit, &normal);
    ...
    svg_set_attr(corner_lbl, "fill", "#ff0033");
    svg_set_content(corner_lbl, "🔱 ZKAEDI GOVERNANCE: HOSTILE TAKEOVER / TIMELOCK EXPLOIT ACTIVE");
    svg_add_child(svg, corner_lbl);

    char* xml = svg_to_string(svg);
    FILE* fp = fopen("test_evm_governance.svg", "w");
    if (fp) {
        fputs(xml, fp);
        fclose(fp);
        printf("SUCCESS: Generated test_evm_governance.svg!\n");
    }
