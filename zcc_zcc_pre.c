...
void __builtin_va_start(__va_list_struct ap, ...);
void __builtin_va_end(__va_list_struct ap);
...
struct PassResult {
    unsigned int order[2048   ];  /* block IDs in emission order */
...
struct Node *node_lhs(struct Node *n) { return n ? n->lhs : ((void*)0); }
struct Node *node_rhs(struct Node *n) { return n ? n->rhs : ((void*)0); }
struct Node *node_cond(struct Node *n) { return n ? n->cond : ((void*)0); }
struct Node *node_then_body(struct Node *n) { return n ? n->then_body : ((void*)0); }
struct Node *node_else_body(struct Node *n) { return n ? n->else_body : ((void*)0); }
struct Node *node_body(struct Node *n) { return n ? n->body : ((void*)0); }
struct Node *node_init(struct Node *n) { return n ? n->init : ((void*)0); }
struct Node *node_inc(struct Node *n) { return n ? n->inc : ((void*)0); }
int node_compound_op(struct Node *n) { return n ? n->compound_op : 0; }
struct Node **node_stmts(struct Node *n) { return n ? n->stmts : ((void*)0); }
...
struct Node *node_arg(struct Node *n, int i) {
    if (!n || !n->args || i < 0 || i >= n->num_args) return ((void*)0);
    return n->args[i];
}
int node_num_args(struct Node *n) { return n ? n->num_args : 0; }
struct Node **node_cases(struct Node *n) { return n ? n->cases : ((void*)0); }
int node_num_cases(struct Node *n) { return n ? n->num_cases : 0; }
struct Node *node_default_case(struct Node *n) { return n ? n->default_case : ((void*)0); }
long long node_case_val(struct Node *n) { return n ? (long long)n->case_val : 0; }
struct Node *node_case_body(struct Node *n) { return n ? n->case_body : ((void*)0); }
...
static void pp_expand_ident(PPState *state, const char *ident) {
    ...
    if (!m->is_function_like) {
        m->active = 0;
        pp_push_input(state, m->body, ((void*)0), m);
        return;
    }
    ...
    for (i = 0; i < p_count; i++) { /* using p_count which is correctly set */
        ...
        pp_push_input(state, args[i], ((void*)0), ((void*)0));
        pp_parse_target_depth(state, state->input_depth);
    ...
    if (cc->tk == TK_IDENT) {
        ...
        if (strcmp(cc->tk_text, "yypParser") == 0 || strcmp(cc->tk_text, "pParser") == 0) {
            printf("DEBUG: parse_primary lookup '%s'. sym=%p, sym->type=%p, base=%p, base_tag='%s'\n", 
                   cc->tk_text,
                   (void*)sym, sym ? (void*)sym->type : ((void*)0), 
                   (sym && sym->type) ? (void*)sym->type->base : ((void*)0), 
        ...
        if (cc->tk == TK_DOT) {
            ...
            if (n->type) {
                ...
                if (f) {
                    ...
                    char buf[256];
                    sprintf(buf, "unknown struct member '%s' in struct '%s' (type=%p, fields=%p)", cc->tk_text, (n->type && n->type->tag[0]) ? n->type->tag : "<anon>", (void*)n->type, n->type ? (void*)n->type->fields : ((void*)0));
                    error(cc, buf);
        ...
        if (cc->tk == TK_ARROW) {
            ...
            if (deref->type) {
                ...
                if (f) {
                    ...
                    char buf[256];
                    sprintf(buf, "unknown struct member '%s' after -> in struct '%s' (type=%p, fields=%p)", cc->tk_text, (deref->type && deref->type->tag[0]) ? deref->type->tag : "<anon>", (void*)deref->type, deref->type ? (void*)deref->type->fields : ((void*)0));
                    error(cc, buf);
                    n->type = cc->ty_int;
                }
            } else {
...
static Node *parse_initializer_list(Compiler *cc, int *out_count) {
    ...
    while (cc->tk != TK_EOF) {
        Node *item = ((void*)0);
        if (cc->tk == TK_LBRACE) {
            item = parse_initializer_list(cc, ((void*)0));
        } else {
...
static void emit_local_initializer(Compiler *cc, Node *block, int *cnt, int *cap, Node *base_var, Type *curr_type, Node *init_list, int offset) {
    if (curr_type->kind == TY_ARRAY) {
        ...
        for (int i = 0; i < init_list->num_args && f != ((void*)0); i++, f = f->next) {
            Node *item = init_list->args[i];
            if (item->kind == ND_INIT_LIST) {
...
Node* zcc_build_from_wire(Compiler *cc, const uint8_t *data, size_t len) {
    if (!data || len < sizeof(WireInitNode)) return ((void*)0);

    const WireInitNode *w = (const WireInitNode*)data;
    if (w->magic != 0x5A434349  ) return ((void*)0);
...
ShmRingBuffer* zcc_shm_ring_open(void) {
    ...
    ShmRingBuffer* ring = (ShmRingBuffer*)mmap(((void*)0), sizeof(ShmRingBuffer),
                                               1 | 2 /* PROT_READ | PROT_WRITE */, 1, fd, 0);
    close(fd);
    return ring;
}
...
Node* zcc_shm_ring_read(Compiler *cc, ShmRingBuffer* ring) {
    ...
    if (head == tail) return ((void*)0);   // empty
...
/* Default weak implementation for C-only builds. 
 ...
 void zcc_rust_free_init_tree(ZccInitNode* root) {
    if (!root) return;
...
static Node* zcc_build_init_internal(Compiler* cc, ZccInitNode* root, Type* target_type) {
    ...
    if (root->kind == ZCC_INIT_VALUE) {
        ...
        if (node->num_args > 0) {
            ...
            for (int i = 0; i < root->list.count; i++) {
                node->args[i] = zcc_build_init_internal(cc, root->list.children[i], ((void*)0));
            }
        }
    }
    return node;
 ...
 *   ADAPT-4  Node struct field names: kind, lhs, rhs, ty, line, next  (ast_*
 * layer) ADAPT-5  Member struct: name, next, offset, ty  (member lookup)
...
struct ZccSymbol {
  char name[256]; /* null-terminated symbol name */
  int kind;                    /* ZCC_SYM_* */
...
int sym_define(ZccScope *sc, const char *name, int kind, Type *ty, int offset) {
  ...
  if (existing) {
    fprintf(stderr, "zcc: sym_define: redefinition of '%s' at depth %d\n", name,
            sc->depth);
    return 0;
  }
...
int tag_complete_type(ZccTagEntry *e, Type *ty) {
  ...
  if (!e || !ty)
    return 0;
  if (e->is_complete) {
    fprintf(stderr, "zcc: tag_complete_type: '%s' already complete\n", e->name);
    return 0;
  }
  e->ty = ty;
  e->is_complete = 1;
...
ZccTagEntry *tag_forward_declare(ZccTagTable *tt, const char *name, int kind) {
  ((void)0);
  return tag_define_internal(tt, name, kind, ((void*)0));
}
...
int type_sizeof_recursive(Type *ty) {
  ...
  default:
    fprintf(stderr, "zcc: type_sizeof_recursive: unknown kind %d\n", ty->kind);
    return 0;
  }
}
...
Type *type_usual_arith_conv(Type *a, Type *b) {
  ...
  if (a->kind == TY_DOUBLE || b->kind == TY_DOUBLE) { /* ADAPT-3 */
...
Type *type_lvalue_conversion(Type *ty) {
  ...
  if (!ty)
    return ((void*)0);
  if (ty->kind == TY_ARRAY)
    return type_decay_array(ty); /* ADAPT-3 */
  if (ty->kind == TY_FUNC)
...
static void ast_walk_dispatch(Node *node, ZccASTVisitor fn, void *ctx,
  ...
  if (!node || !fn)
    return;
  if (!post)
    fn(node, ctx); /* pre-order: visit before children */
  ...
  case 107:
     ...
     *   { Node *a = node->args; while (a) { ast_walk_dispatch(a, fn, ctx,
     * post); a = a->next; } }
     */
    break;
  case 109:
  ...
  if (post)
    fn(node, ctx); /* post-order: visit after children */
}
...
Node *ast_rewrite_expr_recursive(Node *expr, ZccASTRewriter fn, void *ctx) {
   ...
   *   if (expr->lhs) expr->lhs = ast_rewrite_expr_recursive(expr->lhs, fn,
   * ctx); if (expr->rhs) expr->rhs = ast_rewrite_expr_recursive(expr->rhs, fn,
   * ctx);
   */
  return fn(expr, ctx);
}
...
Node *ast_rewrite_stmt_recursive(Node *stmt, ZccASTRewriter fn, void *ctx) {
   ...
   *   if (stmt->body) stmt->body = ast_rewrite_stmt_recursive(stmt->body, fn,
   * ctx); if (stmt->cond) stmt->cond = ast_rewrite_expr_recursive(stmt->cond,
   * fn, ctx);
   */
  return fn(stmt, ctx);
}
...
void codegen_func(Compiler *cc, Node *func) {
  ...
  if (!backend_ops) {
  Type *ret_type = func->func_type ? func->func_type->ret : ((void*)0);
...
ir_node_t *ir_find_def(ir_func_t *fn, ir_node_t *end_node, const char *vreg) {
    ir_node_t *n;
    ir_node_t *def = ((void*)0);
    if (!vreg || !vreg[0]) return ((void*)0);
...
typedef struct LiveInterval {
    char      name[64]; /* %tN name of the temp              */
