...
Type *type_usual_arith_conv(Type *a, Type *b) {
  ...
  a_unsigned = (a->kind == ZCC_TY_UCHAR || a->kind == ZCC_TY_USHORT ||
                a->kind == ZCC_TY_UINT || a->kind == ZCC_TY_ULONG ||
                a->kind == ZCC_TY_ULLONG);
  b_unsigned = (b->kind == ZCC_TY_UCHAR || b->kind == ZCC_TY_USHORT ||
                b->kind == ZCC_TY_UINT || b->kind == ZCC_TY_ULONG ||
...
void ir_emit_lvalue_addr(struct IRAsmCtx *ctx, Node *expr) {
  ...
  fprintf(ctx->out, "  %s = addr ; ADAPT: %s source lvalue\n", tmp,
          "???");              /* ADAPT-4: emit actual lvalue source */
...
void ir_emit_cast(struct IRAsmCtx *ctx, const char *src_tmp, Type *from_ty,
  ...
  if (!ctx || !src_tmp || !from_ty || !to_ty || !dst_out)
    return;
  from_sz = type_sizeof_recursive(from_ty);
  to_sz = type_sizeof_recursive(to_ty);
  from_unsigned =
      (from_ty->kind == ZCC_TY_UCHAR || from_ty->kind == ZCC_TY_USHORT ||
       from_ty->kind == ZCC_TY_UINT || from_ty->kind == ZCC_TY_ULONG ||
...
void ir_emit_call_args(struct IRAsmCtx *ctx, Node *arg_list, int *n_args_out) {
  ...
  while (arg) {
    ...
    fprintf(ctx->out, "  arg %s %s\n",
            arg->ZCC_N_TY ? ir_type_name(ir_map_type(arg->ZCC_N_TY)) : "i32",
            tmp); /* ADAPT-4: arg->type (ZCC_N_TY) */
    n++;
...
int ir_validate_module(struct Function **funcs, int n_funcs) {
  ...
  for (i = 0; i < n_funcs; i++) {
    ...
    if (!ir_validate_func(funcs[i])) {
      fprintf(stderr, "zcc: ir_validate_module: func[%d] failed\n", i);
      ok = 0;
    }
  }
  return ok;
...
int ir_validate_func(struct Function *fn) {
   ...
   * 0; } for (i = 0; i < fn->n_blocks; i++) { BasicBlock *bb = &fn->blocks[i];
   *       if (bb->n_instrs == 0 ||
   * !is_terminator(bb->instrs[bb->n_instrs-1].op)) return 0;
   *   }
   */
...
void zcc_coverage_hit(const char *tag, const char *file, int line) {
  ...
  if (g_zcc_coverage_n >= ZCC_COVERAGE_MAX_HITS)
    return;
...
void zcc_coverage_report(void) {
  int i;
  fprintf(stderr, "=== ZCC COVERAGE REPORT (%d hits, %d unique) ===\n",
          g_zcc_coverage_n, g_zcc_coverage_n);
  for (i = 0; i < g_zcc_coverage_n; i++) {
    fprintf(stderr, "  [%4d]  %-22s  %s:%d\n", g_zcc_coverage_hits[i].count,
            g_zcc_coverage_hits[i].tag, g_zcc_coverage_hits[i].file,
