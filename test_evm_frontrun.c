...
static EvmNode nodes[NUM_NODES] = {
    {{0.0f, 15.0f, 0.0f}, 4.0f, "0x01_MEMPOOL_SCAN", "GAS_PRICE: 250 GWEI", "#00ffcc"}, // Mempool (Cyan)
    {{-12.0f, 2.0f, -6.0f}, 3.8f, "0x12_FRONTRUN_BUY", "EXCHANGE / TOK_BUY", "#ffaa00"}, // Frontrun (Orange)
    {{0.0f, -5.0f, 0.0f}, 4.8f, "0x25_VICTIM_SWAP", "EXCHANGE / SLIPPAGE_MAX", "#ff007f"}, // Victim swap (Pink)
    {{12.0f, 2.0f, 6.0f}, 3.8f, "0x38_BACKRUN_SELL", "EXCHANGE / TOK_SELL", "#ff00ff"}, // Backrun (Magenta)
    {{0.0f, -16.0f, 0.0f}, 4.2f, "0x4A_MEV_PROFIT", "COLLECT / REVENUE_ETH", "#00ff66"} // Profit (Green)
};

#define NUM_EDGES 5
static EvmEdge edges[NUM_EDGES] = {
    {0, 1, 0.7f}, // Mempool -> Frontrun
    {1, 2, 0.8f}, // Frontrun -> Victim (sandwich squeeze)
    {2, 3, 0.8f}, // Victim -> Backrun
    {3, 4, 0.7f}, // Backrun -> Profit
    {4, 0, 0.6f}  // Profit -> Mempool (capital recycling)
};
...
static void get_node_pos(int index, float phase, Vec3* pos, float* radius) {
    ...
    *pos = base.pos;
    ...
    if (index == 2) { // VICTIM (Squeezed between both frontrun and backrun nodes)
        *radius = base.radius * (1.0f + 0.22f * sinf(phase * 4.0f * M_PI));
    } else if (index == 1 || index == 3) {
        // Frontrun and Backrun perform an intertwined double helix orbit
        ...
        // Other nodes float along vertical axes
        pos->y = base.pos.y + 2.0f * sinf(phase * 2.0f * M_PI + (float)index);
    }
}

static float mev_scene_sdf(Vec3 p, ChaosPhasor* ph) {
    Vec3 pr = p;
    ...
    // 3. Double intertwined helix capsules representing the sandwich squeeze
    Vec3 a1 = anim_pos[1]; // Frontrun
    Vec3 b1 = anim_pos[3]; // Backrun
    float double_helix = sdCapsule(pr, a1, b1, 0.9f);
...
static Vec3 mev_get_normal(Vec3 p, ChaosPhasor* ph) {
    Vec3 e = {EPSILON, 0.0f, 0.0f};
    Vec3 n = {
        mev_scene_sdf((Vec3){p.x + e.x, p.y, p.z}, ph) - mev_scene_sdf((Vec3){p.x - e.x, p.y, p.z}, ph),
        mev_scene_sdf((Vec3){p.x, p.y + e.x, p.z}, ph) - mev_scene_sdf((Vec3){p.x, p.y - e.x, p.z}, ph),
        mev_scene_sdf((Vec3){p.x, p.y, p.z + e.x}, ph) - mev_scene_sdf((Vec3){p.x, p.y, p.z - e.x}, ph)
    };
    normalize(&n);
    return n;
}

static float mev_ray_march(Ray r, ChaosPhasor* ph, Vec3* hit, Vec3* normal) {
    float depth = 0.0f;
    for (int i = 0; i < MAX_STEPS; ++i) {
        Vec3 p = {r.ro.x + r.rd.x * depth, r.ro.y + r.rd.y * depth, r.ro.z + r.rd.z * depth};
        float dist = mev_scene_sdf(p, ph);
        depth += dist;
        if (dist < SURF_DIST) {
            *hit = p;
            *normal = mev_get_normal(p, ph);
            return depth;
        }
...
int main() {
    ...
    for (int f = 0; f < num_frames; f++) {
        ...
        for (int y = 0; y < height; y += grid_step) {
            for (int x = 0; x < width; x += grid_step) {
                ...
                Vec3 hit, normal;
                float d = mev_ray_march((Ray){ro, rd}, &ph, &hit, &normal);
    ...
    svg_set_content(corner_lbl, "🔱 ZKAEDI MEV_SANDWICH: DECENTRALIZED POOL FRONT-RUN EXPLOIT DETECTED");
    svg_add_child(svg, corner_lbl);

    char* xml = svg_to_string(svg);
    FILE* fp = fopen("test_evm_frontrun.svg", "w");
    if (fp) {
        fputs(xml, fp);
        fclose(fp);
        printf("SUCCESS: Generated test_evm_frontrun.svg!\n");
    }
