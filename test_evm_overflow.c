...
static EvmNode nodes[NUM_NODES] = {
    {{0.0f, 0.0f, 0.0f}, 4.5f, "0x00_MATH_INIT", "INITIAL: uint256_MAX", "#ffaa00"}, // Math hub (Orange)
    {{-15.0f, 10.0f, -5.0f}, 3.8f, "0x12_ADD_ONE", "ADD 1 (NO_SAFE_MATH)", "#00ffff"}, // Addition (Cyan)
    {{16.0f, 12.0f, 6.0f}, 5.5f, "0x28_OVERFLOW_TRIGGER", "OVERFLOW / WRAP_TO_ZERO", "#ff007f"}, // Overflow target (Hot Pink)
    {{10.0f, -12.0f, -6.0f}, 3.8f, "0x3C_CHECK_REDUCE", "REQUIRE (BAL == 0)", "#ff00ff"}, // Wrap check (Magenta)
    {{-14.0f, -10.0f, 8.0f}, 4.2f, "0x4E_DRAIN_VAULT", "WITHDRAW / DRAINING", "#ff0033"} // Drain (Red)
};

#define NUM_EDGES 5
static EvmEdge edges[NUM_EDGES] = {
    {0, 1, 0.7f}, // Init -> Add
    {1, 2, 0.8f}, // Add -> Overflow (expansion triggering!)
    {2, 3, 0.8f}, // Overflow -> Wrap check
    {3, 4, 0.7f}, // Wrap check -> Drain
    {4, 0, 0.6f}  // Drain -> Init
};
...
static void get_node_pos(int index, float phase, Vec3* pos, float* radius) {
    ...
    *pos = base.pos;
    ...
    if (index == 2) { // OVERFLOW_TRIGGER (Explodes and shrinks rapidly like a supernova!)
...
static float overflow_scene_sdf(Vec3 p, ChaosPhasor* ph) {
    Vec3 pr = p;
    ...
    // 3. Central growing Mandelbulb fractal representing the math overflow
...
static Vec3 overflow_get_normal(Vec3 p, ChaosPhasor* ph) {
    Vec3 e = {EPSILON, 0.0f, 0.0f};
    Vec3 n = {
        overflow_scene_sdf((Vec3){p.x + e.x, p.y, p.z}, ph) - overflow_scene_sdf((Vec3){p.x - e.x, p.y, p.z}, ph),
        overflow_scene_sdf((Vec3){p.x, p.y + e.x, p.z}, ph) - overflow_scene_sdf((Vec3){p.x, p.y - e.x, p.z}, ph),
        overflow_scene_sdf((Vec3){p.x, p.y, p.z + e.x}, ph) - overflow_scene_sdf((Vec3){p.x, p.y, p.z - e.x}, ph)
    };
    normalize(&n);
    return n;
}

static float overflow_ray_march(Ray r, ChaosPhasor* ph, Vec3* hit, Vec3* normal) {
    float depth = 0.0f;
    for (int i = 0; i < MAX_STEPS; ++i) {
        Vec3 p = {r.ro.x + r.rd.x * depth, r.ro.y + r.rd.y * depth, r.ro.z + r.rd.z * depth};
        float dist = overflow_scene_sdf(p, ph);
        depth += dist;
        if (dist < SURF_DIST) {
            *hit = p;
            *normal = overflow_get_normal(p, ph);
            return depth;
        }
...
int main() {
    ...
    ZccSvgNode* stop1 = svg_stop(); svg_set_offset(stop1, "0%"); svg_set_stop_color(stop1, "#ffaa00"); svg_add_child(glowGrad, stop1);
    ...
    for (int f = 0; f < num_frames; f++) {
        ...
        for (int y = 0; y < height; y += grid_step) {
            for (int x = 0; x < width; x += grid_step) {
                ...
                Vec3 hit, normal;
                float d = overflow_ray_march((Ray){ro, rd}, &ph, &hit, &normal);
    ...
    svg_set_attr(corner_lbl, "fill", "#ffaa00");
    svg_set_content(corner_lbl, "🔱 ZKAEDI OVERFLOW: ARITHMETIC WRAP / VAULT LEAK DETECTED");
    svg_add_child(svg, corner_lbl);

    char* xml = svg_to_string(svg);
    FILE* fp = fopen("test_evm_overflow.svg", "w");
    if (fp) {
        fputs(xml, fp);
        fclose(fp);
        printf("SUCCESS: Generated test_evm_overflow.svg!\n");
    }
